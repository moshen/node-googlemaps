...
module.exports = function(params, callback) {
  ...
  if (this.config.key == null) {
    return callback(new Error('The placeTextSearch API requires a key. You can add it to the config.'));
  }
  ...
  var args = _assignParams({}, params, ACCEPTED_PARAMS['place-search-text']);

  if (args.query == null) {
    return callback(new Error('params.query is required'));
  }

  return _makeRequest(this.request, this.config, GOOGLEMAPS_ENDPOINTS['place-search-text'], args, _jsonParser(callback));
