...
var directionOptionalParamNames = ['mode','waypoints','alternatives','avoid','language','units','region','departure_time','arrival_time'];
exports.directions = function(origin, destination, callback, sensor, modeOrOptionalParams, waypoints, alternatives, avoid, units, language) {
  var args = {
    'origin': origin,
  ...
  if (modeOrOptionalParams) {
    if (typeof modeOrOptionalParams === 'string') {
      //preserve backward compatibility
      args.mode = modeOrOptionalParams;
    ...
    else if (Object.prototype.toString.call(modeOrOptionalParams) == '[object Object]') {
      //optional parameters can be accepted as a hash (in the form of an object literal)
      var optParams = modeOrOptionalParams;
      for (var idx = 0; idx < directionOptionalParamNames.length; ++idx) {
        var paramKey = directionOptionalParamNames[idx];
        var paramVal = optParams[paramKey];
        if (paramVal) {
