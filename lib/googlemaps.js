...
var GOOGLEMAPS_ENDPOINTS = {
  ...
  'places-search':   '/maps/api/place/nearbysearch/json',
...
/**
 * Returns a type validated object based a key, type mask object
 */
...
GoogleMapsAPI.prototype.placeSearch = function(params, callback) {
  ...
  if (this.config['console-key'] == null) {
    return callback(new Error('The placeSearch API requires a console-key. You can add it to the config.'));
  }
  ...
  var args = _setParams(params, args, ACCEPTED_PARAMS['places-search']);

  if (args.location == null) {
    return callback(new Error('param.locaiton is required'));
  }
  ...
  */
  ...
  return _makeRequest(this.request, this.config, GOOGLEMAPS_ENDPOINTS['places-search'], args, _returnObjectFromJSON(callback));
};
