...
exports.directions = function(origin, destination, callback, options) {
  var args = {
    'origin': origin,
  ...
  if (options.mode) args.mode = options.mode.toLowerCase();

  // for mode transit you MUST pass either departur or arrival time
  if (options.mode === 'transit' && ((typeof departureTime === "undefined" || departureTime === null) && (typeof arrivalTime === "undefined" || arrivalTime === null))) {
  ...
  if (typeof departureTime !== "undefined" && departureTime !== null) {
    if (options.mode === 'transit' || options.mode === 'driving') {
  ...
  if (options.waypoints) args.waypoints = options.waypoints;
  if (options.alternatives) args.alternatives = options.alternatives;
  if (options.avoid) args.avoid = options.avoid;
  if (options.units) args.units = options.units;
