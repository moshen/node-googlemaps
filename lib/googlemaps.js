...
exports.staticMap = function(center , zoom , size , sensor , callback , maptype , markers){
	var args = {
		'center': center,
		'zoom': zoom,
		'size': size
	...
	if(markers){
		...
		for(i=0; i < markers.length; i++) {
			if(markers[i]['size']){ markers_arg += '|size:' + markers[i]['size'] }
			if(markers[i]['color']){ markers_arg += '|color:' + markers[i]['color'] }
			if(markers[i]['label']){ markers_arg += '|label:' + markers[i]['label'] }
			if(markers[i]['location']){ markers_arg += '|' + markers[i]['location'] }
			args.markers[i] = markers_arg;
		}
	}
	args.sensor = sensor || 'false';
		
	var reqPath = '/maps/api/staticmap?' + qs.stringify(args);
	console.log(reqPath);
