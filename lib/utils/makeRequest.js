...
function _buildUrl(config, args, path) {
  ...
  if (config.google_client_id && config.google_private_key) {
    ...
    var query = querystring.stringify(args).split('');
...
module.exports = function(request, config, path, args, callback, requestMaxLength, encoding) {
  ...
  if (encoding) {
    if (encoding === 'binary' && config.static_map_binary) {
  ...
  request(options, function (error, res, data) {
    ...
    if (res.statusCode === 200) {
      if (config.static_map_warnings) {
        var warning = res.headers && res.headers['x-staticmap-api-warning'];
        if (warning) {
          var warnError = new Error(warning);
          warnError.isWarning = true;
          warnError.code = res.statusCode;
          return callback(warnError, data);
        }
      }
